...
CREATE TABLE `t_sys_tenant_user` (
  `id` VARCHAR(50) NOT NULL COMMENT '主键ID',
  `state` INT(1) NOT NULL DEFAULT 1 COMMENT '状态;1--有效；0--无效',
  `create_time` DATETIME NOT NULL COMMENT '创建时间',
  `tenant_id` VARCHAR(50) NOT NULL COMMENT '企业（组织）ID;和t_sys_corp表中的主键ID关联',
...
CREATE TABLE `t_sys_position_tenant_user` (
  `id` VARCHAR(50) NOT NULL COMMENT '主键ID',
  `tenant_id` VARCHAR(50) NOT NULL COMMENT '租户ID',
  `position_id` VARCHAR(50) NOT NULL COMMENT '职位ID',
  `tenant_user_id` VARCHAR(50) NOT NULL COMMENT '租户用户ID',
  PRIMARY KEY (`id`))
COMMENT = '职位表与租户用户表的关联关系表';
